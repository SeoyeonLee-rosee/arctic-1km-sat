...
# 변수 설정
variable_indices = ["sat"] # ["LDt", "NDt", "NDa", "LNa", "LDa", "LNt"]
#file_types = ["ave", "trend"]

# 저장 경로 설정
input_dir = "../RAW/"
output_dir = "../0.5degree/"
...
lat_target = np.arange(60, 90, 0.5)  # 60~90N, 0.5도 간격
lon_target = np.arange(-180, 180, 0.5)  # -180~180, 0.5도 간격
lon_target, lat_target = np.meshgrid(lon_target, lat_target)

# 마스킹 데이터 로드
mask_data_1 = np.load("../RAW/mask_0.5.npy")  # 마스킹 데이터 (60, 720)
...
for var in variable_indices:
    ...
    for mm in range(stmm, enmm + 1):
        ...
        file_path = os.path.join(input_dir, f"{var}_{mm:02d}.npy")
...
for mm in range(stmm, enmm + 1):
    for var in variable_indices:
        ...
        if mask_data.shape == grid_data.shape:
            ...
            # 저장
        output_file = os.path.join(output_dir, f"{var}_0.5_{mm:02d}.npy")
